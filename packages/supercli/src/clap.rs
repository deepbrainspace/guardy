    ...
    match get_output_style_with_prefix(app_prefix) {
        "color" => {
            Styles::default()
                ...
                .header(Style::new().bold().fg_color(Some(Color::from(StarbaseStyle::Label.ansi_color()))))
                
                // Usage line at the top - use theme-aware property color  
                .usage(Style::new().bold().fg_color(Some(Color::from(StarbaseStyle::Property.ansi_color()))))
                
                // Commands, options, and argument names - use theme-aware shell color
                .literal(Style::new().fg_color(Some(Color::from(StarbaseStyle::Shell.ansi_color()))))
                
                // Placeholders like <VALUE>, [OPTIONS] - use theme-aware muted color
                .placeholder(Style::new().fg_color(Some(Color::from(StarbaseStyle::Muted.ansi_color()))))
                
                // Error messages - use theme-aware failure color
                .error(Style::new().bold().fg_color(Some(Color::from(StarbaseStyle::Failure.ansi_color()))))
                
                // Valid values in help - use theme-aware success color
                .valid(Style::new().fg_color(Some(Color::from(StarbaseStyle::Success.ansi_color()))))
                
                // Invalid values or warnings - use theme-aware caution color
                .invalid(Style::new().fg_color(Some(Color::from(StarbaseStyle::Caution.ansi_color()))))
        },
        "monochrome" => {
            Styles::default()
...
pub fn set_theme_from_env_with_prefix(app_prefix: &str) {
    ...
    if let Ok(theme) = std::env::var(&theme_var) {
        match theme.to_lowercase().as_str() {
            "light" => std::env::set_var("STARBASE_THEME", "light"),
            "dark" => std::env::set_var("STARBASE_THEME", "dark"),
            "auto" => std::env::remove_var("STARBASE_THEME"),
