...
mod tests {
    ...
    fn test_config_loads_defaults() {
        let config = GuardyConfig::load(None, None::<&()>, 0).expect("Should load default config");
    ...
    fn test_config_methods() {
        ...
        unsafe {
            std::env::set_var("GUARDY_TEST_VALUE", "true");
        }
        let test_config = GuardyConfig::load(None, None::<&()>, 0).unwrap();
