...
mod tests {
    ...
    fn create_test_config() -> ScannerConfig {
        ScannerConfig {
            ...
            binary_extensions: vec!["jpg".to_string(), "png".to_string(), "bin".to_string()],
            ..Default::default()
        }
    }
    ...
    fn test_file_info() {
        ...
        let info = File::get_file_info(&test_file).unwrap();
        assert_eq!(info.path, test_file);
        assert_eq!(info.size_bytes, 13);
        assert!(!info.is_likely_binary);
    }
    ...
    fn test_should_process_file() {
        ...
        assert!(File::should_process_file(&text_file, &config));

        // Create a binary file (by extension)
        let binary_file = temp_dir.path().join("test.jpg");
        fs::write(&binary_file, b"\x89PNG\r\n\x1a\n").unwrap();
        assert!(!File::should_process_file(&binary_file, &config)); // Binary files excluded by default
    }
    ...
    fn test_load_file_content() {
        ...
        let loaded_content = File::load_file_content(&test_file, &config).unwrap();
        assert_eq!(loaded_content, test_content);

        // Test file too large (create config with very small limit)
        let small_config = ScannerConfig {
            max_file_size_mb: 0.000001, // Very small limit
            ..config
        };
        let result = File::load_file_content(&test_file, &small_config);
        assert!(result.is_err());
    }

    #[test]
    fn test_process_single_file_empty() {
        let temp_dir = TempDir::new().unwrap();
        let config = create_test_config();
        ...
        let matches = File::process_single_file(&empty_file, &config).unwrap();
