...
mod tests {
    ...
    fn test_statistics() {
        ...
        let txt_file = temp_dir.path().join("test.unknown");
        ...
        // Check both files
        let _ = filter.is_binary_file(&exe_file); // Should be binary by extension
        let _ = filter.is_binary_file(&txt_file); // Should use content inspection
        
        let stats = filter.get_stats();
        assert_eq!(stats.files_checked, 0); // is_binary_file doesn't update files_checked
        
        // Now test through should_filter which updates files_checked
        let _ = filter.should_filter(&exe_file);
        let _ = filter.should_filter(&txt_file);
        
        let stats = filter.get_stats();
        assert_eq!(stats.files_checked, 2);
        assert!(stats.extension_cache_hits >= 1); // .exe should be cache hit
        assert!(stats.content_inspections_performed >= 1); // .unknown should use content inspection
    }
    ...
    fn test_reset_stats() {
        ...
        assert_eq!(stats.files_binary_by_extension, 0);
        assert_eq!(stats.content_inspections_performed, 0);
    }
    ...
    fn test_performance_characteristics() {
        ...
        // Test that shared extensions provide performance benefit
        let test_extensions = ["exe", "dll", "jpg", "png", "zip", "so", "dylib"];
        let test_paths: Vec<_> = test_extensions.iter()
            .map(|ext| format!("test.{}", ext))
            .collect();
        
        let config = create_test_config(false);
        let filter = BinaryFilter::new(&config).unwrap();
        
        // Multiple calls should be fast due to shared HashSet
        let start = Instant::now();
        for _ in 0..1000 {
            for path_str in &test_paths {
                let path = Path::new(path_str);
                let _ = filter.is_binary_file(path);
            }
        }
        let duration = start.elapsed();
        
        // Should complete 7000 extension checks in reasonable time
        assert!(duration.as_millis() < 100, "Binary detection too slow: {:?}", duration);
        
        let stats = filter.get_stats();
        assert!(stats.extension_cache_hits > 0);
    }
}