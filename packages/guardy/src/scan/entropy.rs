...
static HEX_STRING_REGEX: LazyLock<Arc<Regex>> = LazyLock::new(|| {
    Arc::new(Regex::new("^[0-9a-fA-F]{16,}$").unwrap())
});

static CAP_AND_NUMBERS_REGEX: LazyLock<Arc<Regex>> = LazyLock::new(|| {
    Arc::new(Regex::new("^[0-9A-Z]{16,}$").unwrap())
});
