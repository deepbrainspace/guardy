...
mod tests {
    ...
    fn test_progress_stats() {
        ...
        assert_eq!(snapshot.files_with_secrets, 1);
        assert_eq!(snapshot.binary_files, 1);
        assert_eq!(snapshot.files_skipped, 1);
    }
    
    #[test]
    fn test_sequential_progress_creation() {
        let progress = Progress::new_sequential(100).unwrap();
        assert!(!progress.is_parallel);
        assert_eq!(progress.worker_bars.len(), 0);
    }
    
    #[test]
    fn test_parallel_progress_creation() {
        let progress = Progress::new_parallel(100, 4).unwrap();
        assert!(progress.is_parallel);
        assert_eq!(progress.worker_bars.len(), 4);
    }
    
    #[test]
    fn test_factory_functions() {
        let seq_progress = factories::enhanced_sequential_reporter(50);
        assert!(!seq_progress.is_parallel);
        
        let par_progress = factories::enhanced_parallel_reporter(100, 6);
        assert!(par_progress.is_parallel);
        assert_eq!(par_progress.worker_bars.len(), 6);
    }
}