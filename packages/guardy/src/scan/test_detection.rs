...
impl<'a> TestDetector<'a> {
    ...
    fn find_test_block_end(&self, lines: &[&str], start_line: usize, path: &Path) -> Option<usize> {
        ...
        // Check if this line starts a test block
        let is_test_start = self
            .config
            .test_attributes
            .iter()
            .any(|pattern| self.matches_glob_pattern(line, pattern))
            || self
                .config
                .test_modules
                .iter()
...
"#;
        ...
        let found_secrets: Vec<&str> = result.iter().map(|m| m.matched_text.as_str()).collect();

        // Should find regular secrets but not test secrets
        assert!(
            found_secrets
                .iter()
                .any(|s| s.contains("4eC39HqLyjWDarjtT1zdp7dcGGTJ8XA5B9r2F3mQ"))
        );
        assert!(
            found_secrets
                .iter()
                .any(|s| s.contains("5xZ8jM3nK7qW2rT9vY4uL6pS1dF0hC8gA5bE3iO7"))
        );
        assert!(
            found_secrets
                .iter()
                .any(|s| s.contains("8jL5nQ2tY9vX6rB1mF4sK7dA0cG3hE6iO9pZ2wU"))
        );
        ...
        assert!(
            !found_secrets
                .iter()
                .any(|s| s.contains("7nX2mK9qY8dP5vL3wR6tF4uN8hG2cV1sA0eB7iO9"))
        );
        assert!(
            !found_secrets
                .iter()
                .any(|s| s.contains("9rB4mN7qX2sT6vY1uL8pF5dH0cG3kA9bE7iO2wZ"))
        );
        assert!(
            !found_secrets
                .iter()
                .any(|s| s.contains("3mQ6nR9tY2vL5xZ8jK1sF4dC7gA0bE9hO6pW3uN"))
        );
    }
...
"#;
        ...
        assert!(
            found_secrets
                .iter()
                .any(|s| s.contains("2xK8mQ5nR9tY6vL3zJ7dF0hC4gA1bE8iO5pW2uN"))
        );
        assert!(
            found_secrets
                .iter()
                .any(|s| s.contains("3sH6mQ9tY2vX5rB8jL1nF4dK7cA0gE3hO6pZ9wU"))
        );
        ...
        assert!(
            !found_secrets
                .iter()
                .any(|s| s.contains("7qB4nX2sT9vY1uL8pF5dH0cG6kA3bE9hO2pZ5wU"))
        );
        assert!(
            !found_secrets
                .iter()
                .any(|s| s.contains("4mL7nQ0tY3vX6rB9mF2sK5dA8cG1hE4iO7pZ0wU"))
        );
        assert!(
            !found_secrets
                .iter()
                .any(|s| s.contains("6jN9qR2tY5vL8xZ1mK4sF7dC0gA3hE6iO9pW2uN"))
        );
    }
...
"#;
        ...
        assert!(
            !found_secrets
                .iter()
                .any(|s| s.contains("2mQ6nR9tY7vL5xZ8jK1sF4dC7gA0bE9hO6pW3uN"))
        );
        assert!(
            !found_secrets
                .iter()
