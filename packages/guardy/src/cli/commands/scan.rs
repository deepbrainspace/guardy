    ...
    for custom_pattern in &args.custom_patterns {
        match Regex::new(custom_pattern) {
            Ok(regex) => {
                patterns
                    .patterns
                    .push(crate::scanner::patterns::SecretPattern {
    ...
    for path in &scan_paths {
        if path.is_file() {
            ...
            let was_processed = !matches.is_empty()
                ...
                || !crate::scanner::directory::is_binary_file(
                    path,
...
fn group_matches_by_file<'a>(
    matches: &'a [&'a crate::scanner::types::SecretMatch],
) -> Vec<(String, Vec<&'a crate::scanner::types::SecretMatch>)> {
